...
const DPRPage = () => {
  ...
  const fetchDPRData = useCallback(async () => {
    ...
    try {
      ...
      const dischargesOnSelectedDate =
      ...
      const majorOT =
        otProcedures?.filter(
          (ot) => ot.ot_type?.toLowerCase().includes("major") || ot.ot_notes?.toLowerCase().includes("major"),
        ).length || 0

      const minorOT =
        otProcedures?.filter(
          (ot) => ot.ot_type?.toLowerCase().includes("minor") || ot.ot_notes?.toLowerCase().includes("minor"),
        ).length || 0
      ...
      const bedManagementData: BedManagement[] = uniqueRoomTypes
        ...
        .map((roomType) => {
          ...
          return {
            ...
            occupancyRate: Number.parseFloat(occupancyRate.toFixed(1)),
          }
        })
      ...
      setAlerts([
        {
          type: "warning",
          message: "Failed to load data. Please try again later. Check console for details.",
          icon: AlertTriangle,
        },
      ])
    } finally {
        ...
        <style>
          {`
            ...
            * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
            ...
            /* Added secondary KPI grid for better content organization */
            .secondary-kpi-grid {
              display: grid;
              grid-template-columns: repeat(4, 1fr);
              gap: 3mm;
        ...
        <div className="kpi-grid">
          ...
          <div className="kpi">
            ...
            <div className="opd-breakdown">
              ...
              <div className="opd-item">{/* Empty for spacing if needed, or remove if only two sub-points */}</div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-center h-screen bg-gray-50">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">
              Fetching the latest data for{" "}
              <span className="font-semibold">{format(parseISO(selectedDate), "MMM dd, yyyy")}</span>...
            </p>
          </div>
        </div>
      </Layout>
      ...
      <div className="space-y-8 p-6 bg-gray-50 min-h-screen">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 md:gap-6 bg-white p-4 sm:p-6 rounded-lg shadow-md">
          <div>
            ...
            <p className="text-md sm:text-lg text-gray-600 mt-1 sm:mt-2">
              A comprehensive overview of hospital operations for{" "}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between text-center md:text-left">
            <div>
              ...
              <p className="text-blue-100 text-sm sm:text-base">
                Complete overview of all hospital services for {format(parseISO(selectedDate), "EEEE, MMMM dd, yyyy")}
              </p>
            </div>
            <div className="mt-3 md:mt-0 text-center md:text-right">
              <div className="text-3xl sm:text-4xl font-bold">
                {kpiData.totalOPDAppointments + kpiData.totalIPDAdmissions}
              </div>
        ...
        <Card className="shadow-lg border border-gray-100 mt-6">
          <CardHeader className="border-b pb-4">
            <CardTitle className="text-xl font-semibold text-gray-800">
              Service Breakdown Summary ({format(parseISO(selectedDate), "dd-MM-yyyy")})
            </CardTitle>
          </CardHeader>
        ...
        <Card className="shadow-lg border border-gray-100">
          <CardHeader className="border-b pb-4">
            <CardTitle className="text-xl font-semibold text-gray-800">
              Bed Management Overview ({format(parseISO(selectedDate), "dd-MM-yyyy")})
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
                ...
                <TableBody className="bg-white divide-y divide-gray-200">
                  {bedManagement.map((ward, index) => (
                    <TableRow key={index} className="hover:bg-gray-50 transition-colors duration-200">
                      ...
                      <TableCell className="px-4 py-3 whitespace-nowrap text-sm text-green-600 font-semibold">
                        {ward.occupiedBeds}
                      </TableCell>
                      <TableCell className="px-4 py-3 whitespace-nowrap text-sm text-blue-600 font-semibold">
                        {ward.availableBeds}
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        <div style={{ position: "absolute", left: "-9999px", top: "-9999px", width: "210mm", height: "297mm" }}>
          <div ref={printContentRef}>{generatePrintContent()}</div>
        </div>
      </div>
    </Layout>
  )
